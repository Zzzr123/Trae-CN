...
def main():
    ...
    print("=== LLM Chat Interface with Chained Tool Calls (Practice07) ===")
...
示例：
用户请求："查找practice06目录下所有包含'def'关键词的文件，并总结这些文件的主要内容"
步骤1：调用search_files_with_keyword搜索文件
步骤2：调用read_file_content读取每个文件
步骤3：总结内容并返回

用户请求："读取practice06/1.txt和practice06/2.txt两个文件，把两个数相加的和写入result.txt文件"
步骤1：调用read_file_content读取practice06/1.txt
步骤2：调用read_file_content读取practice06/2.txt
步骤3：计算两个数的和
步骤4：调用write_file将结果写入result.txt

用户请求："访问网页并总结内容，保存到文件"
步骤1：调用curl_get访问网页
步骤2：总结网页内容
步骤3：调用write_file保存到文件
